...
os.environ['MCP_CLIENT_CWD'] = '/tmp/test'
os.environ['TRANSFORMERS_CACHE'] = '/tmp/.cache/huggingface'

# Add the server path to sys.path so we can import it
sys.path.insert(0, '/home/vbponomarev/claude-self-reflect/mcp-server/src')
...
async def test_startup_initialization():
    ...
    mcp_server = await server.create_server()
    ...
    if server.model_initialization_task.done():
        ...
        try:
            result = server.model_initialization_task.result()
    ...
    try:
        ...
        embedding2 = await server.generate_embedding("test2")
